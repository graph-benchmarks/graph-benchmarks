...
def load_data(configs, sess: gs.Session, vertex_file: str, edge_file: str):
    ...
    volumes = [
        client.V1Volume(
            name="scratch", nfs=client.V1NFSVolumeSource(path="/scratch", server=nfs_ip)
        ),
        client.V1Volume(
            name="bench-storage",
            nfs=client.V1NFSVolumeSource(path="/bench-storage", server=nfs_ip),
        ),
    ]
    ...
    spec = client.V1JobSpec(
        template=template, backoff_limit=0, ttl_seconds_after_finished=0
    )
    ...
    v = loader.Loader(
        f"hdfs://hadoop-hadoop-hdfs-nn:9000{vertex_file}",
        header_row=False,
        delimiter=" ",
    )
    e = loader.Loader(
        f"hdfs://hadoop-hadoop-hdfs-nn:9000{edge_file}", header_row=False, delimiter=" "
    )
...
def main():
    ...
    for entry in id_algos:
        ...
        if dur > 0:
            log_metrics_sql(
                conn, entry[0], entry[1], dataset, "runtime", dur, vertex, edge, nodes
            )
